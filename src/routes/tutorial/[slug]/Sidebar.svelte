...
<section>
	...
	>
		...
		{#if exercise.next}
			<p><a href="/tutorial/{exercise.next.slug}">Próximo: {exercise.next.title}</a></p>
		{/if}
	</div>
	...
	<footer>
		<a
			...
			rel="noreferrer"
			class="edit"
			href="https://github.com/nazarepiedady/learn-svelte-pt/tree/main/{exercise.dir}"
		>
			Editar esta página
		</a>
	</footer>
...
{#if show_modal}
	<Modal on:close={() => (show_modal = false)}>
		<div class="modal-contents">
			<h2>Copiar e colar está atualmente desligado!</h2>

			<p>
				Nós recomendados digitar o código no editor para completar o exercício, uma vez que isto resulta na melhor retenção e entendimento.
			</p>
			<label>
				<input
					type="checkbox"
				...
				/>
				ativar copiar e colar para a duração desta sessão
			</label>
